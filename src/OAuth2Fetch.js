...
export const oauth2Fetch = async function (req) {
...
async function authorizedFetch(req) {
  if (typeof req.oauth2 === "undefined") {
  ...
  if (!req.oauth2.tokens.access) {
    ...
    return authorizedFetch(req); // redo the original request, now with access token
...
async function tokenFetch(req) {
  if (req.oauth2.grant_type === "authorization_code" && !req.oauth2.tokens.authorization) {
...
function getAuthTokenURL(urlEndpoint, req) {
...
function getAccessTokenURL(urlEndpoint, req) {
  let url = new URL(urlEndpoint);
  url.hash = ""; //disallowed by spec (#3.1)
  let params = {
...
function getAuthorizedRequest(req) {
  ...
  result.headers.Authorization =
    req.oauth2.tokens.access.type + " " + req.oauth2.tokens.access.value;
